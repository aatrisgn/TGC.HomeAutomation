...
export class DeviceChartComponent implements OnInit {
  ...
  observableData$:Subject<LineChartData[]>;
  individualData$:Subject<LineChartData[]>;
  ...
  multiDeviceData: { [deviceId: string]: LineChartData[] } = {}; // Store data for multiple devices

  constructor(
    private readonly deviceClient:DeviceClient,
    private readonly measureClient:MeasureClient
  ) {
    this.observableData$ = new Subject<LineChartData[]>();
    this.individualData$ = new Subject<LineChartData[]>();
  ...
  keepData(): void {
    ...
    this.observableData$.next(this.temporaryDataStorage);
  }
  ...
  fetchDeviceData(): void {
    ...
    this.measureClient
      .getMeasuresByDeviceIdMeasureTypeAndDate(this.selectedDeviceId, this.measureTypeValue, this.startDate, this.endDate)
      .subscribe(data => {
        let convertedData = this.groupMeasurementsByType(data.measures!);
        this.chartData = convertedData;

        this.individualData$.next(this.chartData);
      });
  }
