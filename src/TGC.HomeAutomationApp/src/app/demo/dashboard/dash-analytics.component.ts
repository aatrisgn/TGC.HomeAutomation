...
@Component({
  ...
  standalone: true,
  imports: [SharedModule, NgApexchartsModule, DeviceTableComponent, DeviceTableComponent, TemperatureSummaryComponent],
...
export default class DashAnalyticsComponent {
  ...
  constructor(private deviceClient:DeviceClient, private measureClient:MeasureClient) {
    ...
    measureClient.getCurrentInside("temperature").subscribe(response => {
      this.indoorTemperature = response.dataValue ?? 0;
    });

    measureClient.getCurrentOutside("temperature").subscribe(response => {
      this.outdoorTemperature = response.dataValue ?? 0;
    });

    measureClient.getCurrentInside("humidity").subscribe(response => {
      this.indoorHumidity = response.dataValue ?? 0;
    });

    measureClient.getCurrentOutside("humidity").subscribe(response => {
      this.outdoorHumidity = response.dataValue ?? 0;
    });
